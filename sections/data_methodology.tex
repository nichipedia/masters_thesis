...
The selection was driven by a genetic algorithm that was used for speed as ooposed to accuracy.
...
Data derived from predicted \ac{EGM} datasets represents the actual value of bathymetry at that point.
For example, predicted bathymetry datasets will have a coverage.
These coverages vary in size by the resolution of the imagery.
A higher resolution imagery will have smaller coverages, and lower resolution will have larger coverages.
The predicted values used in this project represent a average of bathymetry across a coverage. 
...
Using this data is not intended to build accurate predictors, but to show that a accurate predictor can be built.

\par
The \ac{ETOPO} dataset is used in this project for bathymetry data.
This dataset is a aggregation of sparse \ac{MBES} ship soundings and predicted bathymetry from a \ac{EGM}.
...
To accomodate this requirement the \ac{ETOPO} dataset was binned into discrete classes.
...
All feature data was aggregated from ocean and earth studies then normalized and formatted for this project.
...
This approach is fundamentally differnt to building a physics model.
...
For this project the Earth is such a globe and a map is a excellent example of how lattitude and longitude lines for a grid pattern.
